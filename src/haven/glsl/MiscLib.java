    ...
    public static final Uniform maploc = new Uniform.AutoApply(VEC3, PView.loc) {
        public void apply(GOut g, VarID loc) {
            ...
            try {
                ...
                /*
                 * XXX: WaterTile's obfog effect is the only thing that uses maploc, in order to
                 * get the precise water surface level. Arguably, maploc should be eliminated
                 * entirely and the obfog should pass the water level in a uniform instead.
                 * However, this works better for now, because with such a mechanic,
                 * Skeleton.FxTrack audio sprites would never complete if they get outside the
                 * map and stuck as constantly loading and never playing. Either way, when
                 * loading, the likely quite slight deviation between origin-Z and map-Z level
                 * probably doesn't matter a whole lot, but solve pl0x.
                 */
            }
    ...
    public static final Function olblend = new Function.Def(VEC4) {
        {
            ...
            Expression blend = param(IN, VEC4).ref();
            /*
             * XXX: It seems weird indeed that Haven would be the only program having
             * trouble with this, but on Intel GPUs, the sign() function very much appears
             * to be buggy somehow (quite unclear just how; no explanation I can think of
             * seems to make sense). Thus, this uses (x * 1000) instead of sign(x). Assuming
             * color values mapped from uint8_t's, 1000 is enough to saturate the smallest
             * deviations from 0.5.
             */
            code.add(
                    new Return(vec4(
    ...
    public static final Function rgb2hsv = new Function.Def(VEC3, "rgb2hsv") {
        {
            ...
            Expression p = code.local(VEC4, mix(vec4(pick(c, "bg"), l(-1.0), l(2.0 / 3.0)),
                    vec4(pick(c, "gb"), l(0.0), l(-1.0 / 3.0)), step(pick(c, "b"), pick(c, "g")))).ref();
            Expression q = code.local(VEC4, mix(vec4(pick(p, "xyw"), pick(c, "r")), vec4(pick(c, "r"), pick(p, "yzx")),
                    step(pick(p, "x"), pick(c, "r")))).ref();
            Expression d = code.local(FLOAT, sub(pick(q, "x"), min(pick(q, "w"), pick(q, "y")))).ref();
            Expression e = l(1.0e-10);
            code.add(new Return(
                    vec3(abs(add(pick(q, "z"), div(sub(pick(q, "w"), pick(q, "y")), add(mul(l(6.0), d), e)))),
                            div(d, add(pick(q, "x"), e)), pick(q, "x"))));
        }
    };
    ...
    public static final Function hsv2rgb = new Function.Def(VEC3, "hsv2rgb") {
        {
            ...
            Expression p = code
                    .local(VEC3,
                            abs(sub(mul(fract(add(pick(c, "xxx"), vec3(1.0, 2.0 / 3.0, 1.0 / 3.0))), l(6.0)), l(3.0))))
                    .ref();
            code.add(new Return(
                    mul(pick(c, "z"), mix(vec3(l(1.0)), clamp(sub(p, l(1.0)), l(0.0), l(1.0)), pick(c, "y")))));
        }
    };
}
