...
public class RenderList {
    ...
    private static final Comparator<Slot> cmp = new Comparator<Slot>() {
        public int compare(Slot a, Slot b) {
            ...
            if ((ret = a.o.cmp().compare(a.r, b.r, a.os, b.os)) != 0)
                return (ret);
            if ((ret = ((System.identityHashCode(a.r) & 0x7fffffff)
                    - (System.identityHashCode(b.r) & 0x7fffffff))) != 0)
                return (ret);
    ...
    private GLState.Global[] getgstates() {
        ...
        IdentityHashMap<GLState.Global, GLState.Global> gstates = new IdentityHashMap<GLState.Global, GLState.Global>(
                this.gstates.length);
        ...
        for (int i = 0; (i < cur) && list[i].d; i++) {
            ...
            if (c.slots.isEmpty()) {
                if (newcache.get(c) != null) {
                    // throw(new RuntimeException(String.format("statroot for %s already in new
                    // cache even though empty", c.root)));
    ...
    public void fin() {
        ...
        if (trimseq++ > 100) {
            /*
             * XXX: Trimming the slot-list is fairly delicate business due to how it
             * interacts with the GC. Keeping a bloated list forever is obviously
             * detrimental, but trimming old objects can also cause old-gen throughput and
             * consequential full GCs. Substantial performance benefits have also been
             * observed when discarding the list and recreating it each cycle, but only for
             * the cost of more frequent GCs. More experimentation is desired.
             */
    ...
    private void instancify() {
        ...
        while ((i < cur) && list[i].d) {
            ...
            tryinst: {
                /*
                 * XXX: How to handle eyeorder and similar things is... tricky. This is an ugly
                 * hack. Please replace.
                 */
