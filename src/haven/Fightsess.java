...
public class Fightsess extends Widget {
    ...
    public Fightsess(int nact) {
        ...
        for (int i = 0; i < 10; i++)
            keystex[i] = Text.renderstroked(kb_acts[i].key().nameShort(), Color.WHITE, Color.BLACK, Text.num12boldFnd)
                    .tex();
    }
    ...
    public void draw(GOut g) {
...

        {
            ...
            if (now < fv.atkct) {
                ...
                if (Config.showcooldown)
                    g.atextstroked(Utils.fmt1DecPlace(fv.atkct - now), cdc, 0.5, 0.5, Color.WHITE, Color.BLACK,
                            Text.num11Fnd);
            }
        ...
        for (int i = 0; i < actions.length; i++) {
            Coord ca = Config.altfightui ? new Coord(gcx - 18, gui.sz.y - 250).add(actc(i)).add(16, 16)
                    : pcc.add(actc(i));
    ...
    public static final String[] keytips = { "1", "2", "3", "4", "5", "Shift+1", "Shift+2", "Shift+3", "Shift+4",
            "Shift+5" };
    ...
    public static final KeyBinding[] kb_acts = { KeyBinding.get("fgt/0", KeyMatch.forcode(KeyEvent.VK_1, 0)),
    ...
    public static final KeyBinding kb_switch = KeyBinding.get("switch_opp",
            KeyMatch.forcode(KeyEvent.VK_TAB, KeyMatch.C));

    /*
     * XXX: This is a bit ugly, but release message do need to be properly sequenced
     * with use messages in some way.
     */
